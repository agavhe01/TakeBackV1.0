...
@app.get("/api/analytics/spending", response_model=list[SpendingAnalyticsResponse])
async def get_spending_analytics(
    ...
    try:
        ...
        for i, budget in enumerate(budgets):
            # Get card_budget associations for this budget
            card_budgets_response = supabase.table("card_budgets").select("id").eq("budget_id", budget["id"]).execute()
